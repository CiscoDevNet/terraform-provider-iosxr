...
func TestAccDataSourceIosxrSNMPServer(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.iosxr_snmp_server.test", "vrfs.0.contexts.0.context_name", "CONT-VRF-VRF1"))
...
func testAccDataSourceIosxrSNMPServerConfig() string {
	...
	config += `		contexts = [{` + "\n"
	config += `			context_name = "CONT-VRF-VRF1"` + "\n"
	config += `		}]` + "\n"
	config += `	}]` + "\n"
	config += `	users = [{` + "\n"
