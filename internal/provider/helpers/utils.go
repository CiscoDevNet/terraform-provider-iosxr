...
// ============================================================================
// XML Namespace Sibling Utilities
// ============================================================================

// YangNameProvider is an interface for types that provide YANG name access.
// This allows the sibling functions to work with any type that has a YangName field.
type YangNameProvider interface {
	GetYangName() string
}

// IsXmlNamespaceSibling reports whether an attribute must be emitted as a separate
...
func IsXmlNamespaceSiblingStr(yangName string, allYangNames []string) bool {
	// Auto-detect: must have a namespace prefix anywhere in the path.
	if !HasNamespacePrefix(yangName) {
		return false
	}
	myTop := TopElementName(yangName)
	// Check whether any other attribute in the list shares the same root element.
	for _, otherYangName := range allYangNames {
		if otherYangName == yangName {
			continue
		}
		if TopElementName(otherYangName) == myTop {
			return true
		}
	}
	return false
...
// HasXmlNamespaceSiblingsStr returns true when any YANG name in the list needs
// separate XML sibling treatment (either via explicit flag or auto-detection).
func HasXmlNamespaceSiblingsStr(yangNames []string) bool {
	for _, yangName := range yangNames {
		if IsXmlNamespaceSiblingStr(yangName, yangNames) {
			return true
		}
	}
	return false
...
// SiblingGroup holds a set of items that share the same top-level XML
...
// body so they produce one merged XML element rather than duplicate siblings.
...
// GroupXmlSiblingsStr returns the xml-namespace-sibling items from allItems
// collected into groups that share the same top-level element. The order of
// groups follows the first occurrence of each key in allItems.
// The getYangName function extracts the YANG name from each item.
func GroupXmlSiblingsStr[T any](allItems []T, getYangName func(T) string) []SiblingGroup[T] {
	// First collect all yang names
	yangNames := make([]string, len(allItems))
	for i, item := range allItems {
		yangNames[i] = getYangName(item)
	}

	seen := map[string]int{} // key -> index in groups
	var groups []SiblingGroup[T]
	for i, item := range allItems {
		yangName := yangNames[i]
		if !IsXmlNamespaceSiblingStr(yangName, yangNames) {
			continue
		}
		// Build the group key: "<ns-prefix>:<top-element>"
		prefix := XmlNamespacePrefixFromXPath(yangName)
		top := TopElementName(yangName)
		key := prefix + ":" + top
		if prefix == "" {
			key = top
		}
		if idx, ok := seen[key]; ok {
			...
			seen[key] = len(groups)
			groups = append(groups, SiblingGroup[T]{Key: key, Items: []T{item}})
		}
	}
	return groups
}
...
// ============================================================================
// XML Sibling Finding and Creation Functions
// ============================================================================
