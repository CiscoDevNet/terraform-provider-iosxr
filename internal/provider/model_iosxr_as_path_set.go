...
func (data ASPathSet) getXPath() string {
	path := "Cisco-IOS-XR-um-route-policy-cfg:/routing-policy/sets/as-path-sets/as-path-set[set-name=%s]"
	path = fmt.Sprintf(path, fmt.Sprintf("%v", data.SetName.ValueString()))
	return path
}

func (data ASPathSetData) getXPath() string {
	path := "Cisco-IOS-XR-um-route-policy-cfg:/routing-policy/sets/as-path-sets/as-path-set[set-name=%s]"
	path = fmt.Sprintf(path, fmt.Sprintf("%v", data.SetName.ValueString()))
	return path
}
...
func (data ASPathSet) toBodyXML(ctx context.Context, stateArg ...*ASPathSet) string {
	var state *ASPathSet
	if len(stateArg) > 0 {
		state = stateArg[0]
	}
	body := netconf.Body{}
	if !data.Rpl.IsNull() && !data.Rpl.IsUnknown() {
		body = helpers.SetFromXPath(body, data.getXPath()+"/rplas-path-set", data.Rpl.ValueString())
	}
...
func (data *ASPathSet) updateFromBody(ctx context.Context, res gjson.Result) {
	if value := res.Get("rplas-path-set"); value.Exists() && !data.Rpl.IsNull() {
...
func (data *ASPathSet) fromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res.Get(prefix + "rplas-path-set"); value.Exists() {
		data.Rpl = types.StringValue(value.String())
	}
}
...
func (data *ASPathSetData) fromBody(ctx context.Context, res gjson.Result) {
...
func (data *ASPathSet) getEmptyLeafsDelete(ctx context.Context, state *ASPathSet) []string {
...
func (data *ASPathSet) addDeletedItemsXML(ctx context.Context, state ASPathSet, body string) string {
	// Start with an empty body - we'll build up the delete operations
	b := netconf.Body{}
	deletedPaths := make(map[string]bool)
	_ = deletedPaths // Avoid unused variable error when no delete_parent attributes exist
	if !state.Rpl.IsNull() && data.Rpl.IsNull() {
		deletePath := state.getXPath() + "/rplas-path-set"
...
func (data *ASPathSet) addDeletePathsXML(ctx context.Context, body string) string {
	b := netconf.NewBody(body)
	if !data.Rpl.IsNull() {
		b = helpers.RemoveFromXPath(b, data.getXPath()+"/rplas-path-set")
	}
