...
func (data CommunitySet) getXPath() string {
	path := "Cisco-IOS-XR-um-route-policy-cfg:/routing-policy/sets/community-sets/community-set[set-name=%s]"
	path = fmt.Sprintf(path, fmt.Sprintf("%v", data.SetName.ValueString()))
	return path
}

func (data CommunitySetData) getXPath() string {
	path := "Cisco-IOS-XR-um-route-policy-cfg:/routing-policy/sets/community-sets/community-set[set-name=%s]"
	path = fmt.Sprintf(path, fmt.Sprintf("%v", data.SetName.ValueString()))
	return path
}
...
func (data CommunitySet) toBodyXML(ctx context.Context, stateArg ...*CommunitySet) string {
	var state *CommunitySet
	if len(stateArg) > 0 {
		state = stateArg[0]
	}
	body := netconf.Body{}
	if !data.Rpl.IsNull() && !data.Rpl.IsUnknown() {
		body = helpers.SetFromXPath(body, data.getXPath()+"/rpl-community-set", data.Rpl.ValueString())
	}
...
func (data *CommunitySet) updateFromBody(ctx context.Context, res gjson.Result) {
	if value := res.Get("rpl-community-set"); value.Exists() && !data.Rpl.IsNull() {
...
func (data *CommunitySet) fromBody(ctx context.Context, res gjson.Result) {
...
func (data *CommunitySet) getEmptyLeafsDelete(ctx context.Context, state *CommunitySet) []string {
...
func (data *CommunitySet) addDeletedItemsXML(ctx context.Context, state CommunitySet, body string) string {
	// Start with an empty body - we'll build up the delete operations
	b := netconf.Body{}
	deletedPaths := make(map[string]bool)
	_ = deletedPaths // Avoid unused variable error when no delete_parent attributes exist
	if !state.Rpl.IsNull() && data.Rpl.IsNull() {
		deletePath := state.getXPath() + "/rpl-community-set"
...
func (data *CommunitySet) addDeletePathsXML(ctx context.Context, body string) string {
	b := netconf.NewBody(body)
	if !data.Rpl.IsNull() {
		b = helpers.RemoveFromXPath(b, data.getXPath()+"/rpl-community-set")
	}
