...
func (data EtagSet) getPath() string {
	return fmt.Sprintf("Cisco-IOS-XR-um-route-policy-cfg:/routing-policy/sets/etag-sets/etag-set[set-name=%s]", data.SetName.ValueString())
}

func (data EtagSetData) getPath() string {
	return fmt.Sprintf("Cisco-IOS-XR-um-route-policy-cfg:/routing-policy/sets/etag-sets/etag-set[set-name=%s]", data.SetName.ValueString())
}

// getXPath returns the XPath for NETCONF operations
func (data EtagSet) getXPath() string {
	path := "Cisco-IOS-XR-um-route-policy-cfg:/routing-policy/sets/etag-sets/etag-set[set-name=%s]"
	path = fmt.Sprintf(path, fmt.Sprintf("%v", data.SetName.ValueString()))
	return path
}

func (data EtagSetData) getXPath() string {
	path := "Cisco-IOS-XR-um-route-policy-cfg:/routing-policy/sets/etag-sets/etag-set[set-name=%s]"
	path = fmt.Sprintf(path, fmt.Sprintf("%v", data.SetName.ValueString()))
	return path
}
...
func (data EtagSet) toBody(ctx context.Context) string {
	body := "{}"
	...
	if !data.Rpl.IsNull() && !data.Rpl.IsUnknown() {
		body, _ = sjson.Set(body, "etag-set-as-text", data.Rpl.ValueString())
	}
	return body
}
...
func (data EtagSet) toBodyXML(ctx context.Context, stateArg ...*EtagSet) string {
	var state *EtagSet
	if len(stateArg) > 0 {
		state = stateArg[0]
	}
...
func (data *EtagSet) updateFromBody(ctx context.Context, res gjson.Result) {
	if value := res.Get("etag-set-as-text"); value.Exists() && !data.Rpl.IsNull() {
...
func (data *EtagSet) fromBody(ctx context.Context, res gjson.Result) {
...
func (data *EtagSet) getDeletedItems(ctx context.Context, state EtagSet) []string {
...
func (data *EtagSet) getEmptyLeafsDelete(ctx context.Context, state *EtagSet) []string {
...
func (data *EtagSet) getDeletePaths(ctx context.Context) []string {
...
func (data *EtagSet) addDeletedItemsXML(ctx context.Context, state EtagSet, body string) string {
...
func (data *EtagSet) addDeletePathsXML(ctx context.Context, body string) string {
	b := netconf.NewBody(body)
	if !data.Rpl.IsNull() {
		b = helpers.RemoveFromXPath(b, data.getXPath()+"/etag-set-as-text")
	}
