...
func (data EVPNRouteSyncStitchingEVI) toBody(ctx context.Context, providerVersion string) string {
	body := "{}"
...
func (data EVPNRouteSyncStitchingEVI) GetVersionConstraints() []helpers.FieldVersionConstraint {
...
func (data *EVPNRouteSyncStitchingEVI) fromBody(ctx context.Context, res []byte) {
...
func (data *EVPNRouteSyncStitchingEVIData) fromBody(ctx context.Context, res []byte) {
	if value := gjson.GetBytes(res, "description"); value.Exists() {
		data.Description = types.StringValue(value.String())
	}
	if value := gjson.GetBytes(res, "bgp.rd.two-byte-as-number"); value.Exists() {
...
func (data *EVPNRouteSyncStitchingEVI) getEmptyLeafsDelete(ctx context.Context) []string {
	emptyLeafsDelete := make([]string, 0)
	if !data.VrfDefault.IsNull() && !data.VrfDefault.ValueBool() {
		emptyLeafsDelete = append(emptyLeafsDelete, fmt.Sprintf("%v/vrf/default", data.getPath()))
	}
	for i := range data.BgpRouteTargetExportIpv4AddressFormat {
		keys := [...]string{"ipv4-address", "assigned-number"}
		keyValues := [...]string{data.BgpRouteTargetExportIpv4AddressFormat[i].Ipv4Address.ValueString(), strconv.FormatInt(data.BgpRouteTargetExportIpv4AddressFormat[i].AssignedNumber.ValueInt64(), 10)}
