...
func (data EVPNSegmentRoutingSRv6EVI) toBodyXML(ctx context.Context, stateArg ...*EVPNSegmentRoutingSRv6EVI) string {
...
func (data *EVPNSegmentRoutingSRv6EVI) updateFromBody(ctx context.Context, res gjson.Result) {
	...
	for i := range data.Locators {
		...
		var r gjson.Result
		res.Get("locators.locator").ForEach(
			func(_, v gjson.Result) bool {
				found := false
				for ik := range keys {
					if v.Get(keys[ik]).String() == keyValues[ik] {
...
func (data *EVPNSegmentRoutingSRv6EVI) updateFromBodyXML(ctx context.Context, res xmldot.Result) {
	...
	for i := range data.Locators {
		...
		if value := helpers.GetFromXPath(r, "locator-name"); value.Exists() && !data.Locators[i].LocatorName.IsNull() {
			data.Locators[i].LocatorName = types.StringValue(value.String())
		...
		for j := range data.Locators {
			...
			if found {
				...
				if !state.Locators[i].UsidAllocationWideLocalIdBlock.IsNull() && state.Locators[i].UsidAllocationWideLocalIdBlock.ValueBool() && data.Locators[j].UsidAllocationWideLocalIdBlock.IsNull() {
					b = helpers.RemoveFromXPath(b, fmt.Sprintf(state.getXPath()+"/locators/locator%v/usid/allocation/wide-local-id-block", predicates))
				}
				break
			}
		}
		if !found {
			b = helpers.RemoveFromXPath(b, fmt.Sprintf(state.getXPath()+"/locators/locator%v", predicates))
		}
	}
	// For boolean fields, only delete if state was true (presence container was set)
