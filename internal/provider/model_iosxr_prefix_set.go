...
func (data PrefixSet) getXPath() string {
	path := "Cisco-IOS-XR-um-route-policy-cfg:/routing-policy/sets/prefix-sets/prefix-set[set-name=%s]"
	path = fmt.Sprintf(path, fmt.Sprintf("%v", data.SetName.ValueString()))
	return path
}

func (data PrefixSetData) getXPath() string {
	path := "Cisco-IOS-XR-um-route-policy-cfg:/routing-policy/sets/prefix-sets/prefix-set[set-name=%s]"
	path = fmt.Sprintf(path, fmt.Sprintf("%v", data.SetName.ValueString()))
	return path
}
...
func (data PrefixSet) toBodyXML(ctx context.Context, stateArg ...*PrefixSet) string {
	var state *PrefixSet
	if len(stateArg) > 0 {
		state = stateArg[0]
	}
	body := netconf.Body{}
	if !data.Rpl.IsNull() && !data.Rpl.IsUnknown() {
		body = helpers.SetFromXPath(body, data.getXPath()+"/rpl-prefix-set", data.Rpl.ValueString())
	}
...
func (data *PrefixSet) updateFromBody(ctx context.Context, res gjson.Result) {
	if value := res.Get("rpl-prefix-set"); value.Exists() && !data.Rpl.IsNull() {
...
func (data *PrefixSet) addDeletedItemsXML(ctx context.Context, state PrefixSet, body string) string {
	...
	if !state.Rpl.IsNull() && data.Rpl.IsNull() {
		deletePath := state.getXPath() + "/rpl-prefix-set"
...
func (data *PrefixSet) addDeletePathsXML(ctx context.Context, body string) string {
	...
	if !data.Rpl.IsNull() {
		b = helpers.RemoveFromXPath(b, data.getXPath()+"/rpl-prefix-set")
	}
